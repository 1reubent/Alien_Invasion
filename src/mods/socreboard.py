...
class Scoreboard:
    ...
    def prep_ships(self):
        self.lives_image = self.font.render(
            "Lives:", True, self.text_color, self.settings.bg_color
        )
        self.lives_image_rect = self.lives_image.get_rect()
        self.lives_image_rect.x = 10
        self.lives_image_rect.y = self.ai_game.ship.rect.height // 2
        ...
        for ship_number in range(self.stats.ships_left):
            ...
            ship.rect.x = (
                10
                + self.lives_image_rect.width
                + 5
                + ship_number * (ship.rect.width + 5)
            )
            ship.rect.y = 10
            self.ships.add(ship)
    ...
    def prep_score(self):
        ...
        # -1 means round to the nearest 10, -2 means 100 etc.
